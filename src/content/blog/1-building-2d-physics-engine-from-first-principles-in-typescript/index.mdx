...
date: "Feb 18 2026"
image: "../../../assets/blog/physics_engine.png"
color: "#3b82f6"
---
...
The core of the simulation is to evolve this state over time based on physical laws. Newton's second law, $F = ma$, governs motion:

- **Linear Motion**: $F = ma$ (Force = mass × acceleration). We can rearrange this to find the acceleration: $a = F / m$.
- **Rotational Motion**: $\tau = I\alpha$ (Torque = moment of inertia × angular acceleration). Similarly, angular acceleration is: $\alpha = \tau / I$.
...
2.  **Semi-Implicit Euler (or Symplectic Euler)**: A significant improvement in stability with minimal extra cost. It calculates the new velocity first and then uses that *new* velocity to calculate the new position. This prevents the "energy explosion" that can occur with Explicit Euler.
    ...
    *   $v(t + dt) = v(t) + a(t) \cdot dt$
    *   $p(t + dt) = p(t) + v(t + dt) \cdot dt$  **(This is the key difference)**
...
2.  For each axis:
    a. "Project" both shapes onto the axis. This results in two 1D intervals (a min and max value). The projection is calculated using the dot product: $p = V \cdot N$.
...
$$
j = \frac{-(1 + e) \cdot (v_{rel} \cdot n)}{\frac{1}{m_a} + \frac{1}{m_b} + \frac{(r_{ap} \perp n)^2}{I_a} + \frac{(r_{bp} \perp n)^2}{I_b}}
$$
